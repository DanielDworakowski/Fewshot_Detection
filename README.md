...
1) syntax changes were introduced to ensure that integers are returned on division operations in several locations and 2) the initialization procedures for batch normalization as well as convolutional layers were reverted to their state in pytorch 0.3.1. 

The results were verified using the same instructions provided by the paper's authors. 
